...
margin["game_key"] = (
    margin["Season"].astype(str)
    + "_"
    + margin["DayNum"].astype(str)
    + "_"
    + margin["TeamID_1"].astype(str)
    + "_"
...
game_team = game_team.rename(
    columns={"Season_o": "Season", "DayNum_o": "DayNum", "TeamID_o": "TeamID"}
)
...
game_team["game_key"] = (
    game_team["Season"].astype(str)
    + "_"
    + game_team["DayNum"].astype(str)
    + "_"
    + np.minimum(game_team["TeamID"], game_team["Opponent"]).astype(str)
    + "_"
...
game_stats = game_stats.rename(
    columns={c: f"{c}_game" for c in game_stats if c[-2] == "_"}
)
...
season_sum = season_sum.rename(
    columns={c: f"{c}_sum" for c in game_team if c[-2] == "_"},
)
...
def from_sum_to_pct(sum_df, prefix=""):
    ...
    pct_df = pct_df.drop(columns=[f"{prefix}Poss_poss_o", f"{prefix}Poss_poss_d"])
